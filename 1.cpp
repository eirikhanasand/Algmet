 ...
 * is deleted. Tested using solution from 1c.
 * 1c. Created a insert function that inserts the given value at a certain point, 
 * given that the point has been initialized, otherwise places it at the back of
 * the vector. Tested using infinite while loop at the bottom of the code. Prints
 * new size till out of memory.
*/
 ...
 *   @author   Eirik Hanasand, insert(...), resize(...)
 */
...
class Vector {                       //  (finnes ikke direkte i Java!
    ...
    //          for (int i = 0; i < antall; i++)  data[i] = 0;  //  Unødvendig?!!!
        ...
        bool remove(const int pos) {    //  Fjerner (om mulig) element nr.'pos':
            ...
            void resize(const int nyLengde, const int val = 0) {   //  Øker (om mulig) arrayens lengde.
                if (nyLengde > capacity()) {
                    T* temp = new T[nyLengde];

                    for (int i = 0; i < capacity(); i++) {
                        temp[i] = data[i];
                    }

                    for (int i = 0; i < nyLengde-capacity(); i++) {
                        temp[i] = val;
                    }
                    ...
                    cout << "The new size must be larger than the current size.\n\n";
                }
            }
...
int main()  {
    ...
    cout << "Min kode" << endl;
    sVec.push_back("11");    sVec.push_back("11");
    sVec.push_front("22");   sVec.push_front("22");

    sVec.display();
    sVec.remove(1);   sVec.remove(0);
    cout << sVec.capacity() << "\n\n";
    sVec.resize(300);
    cout << sVec.capacity() << "\n\n";
    sVec.clear();
